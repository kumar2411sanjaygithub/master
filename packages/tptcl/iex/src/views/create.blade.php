...
@section('content')
    ...
    <h5><label  class="control-label"><u>{{ (@$permissions->id!='')?'EDIT PERMISSION PACKAGE':'ADD PERMISSION'}}</u></label></h5>
      <ol class="breadcrumb">
...
<div class="box mt3">
  <div class="box-body">
  <form method="post" action="{{ (@$permissions->id!='')?url('/task/update/'.$permissions->id):route('permissionlist.store')}}">
            {{ csrf_field() }}
