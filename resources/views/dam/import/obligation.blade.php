...
@section('content')
   ...
   <h5>
      <label  class="control-label">IMPORT OBLIGATION</label>
   </h5>
   <ol class="breadcrumb">
               ...
               <?php $i=1; ?>
                              @foreach ($obligations as $key=>$obligation)
                                  <tr>
                                    ...
                                    <td class="text-center">{{ $obligation->userclient() }}</td>
                                    

                                    <?php if(count($obligation->oblDetails) > 0){
                                    ?>
                                     <td class="text-center">
                                      <a href="{{ URL('/obligation/import/'.$obligation->id) }}"><span class="glyphicon glyphicon-upload"></span></a>
                                    </td>
...
<script>
        $(document).ready(function() {
            ...
            $('#submit_go').on('click',function(event){
                ...
                window.location = "{{ route('obligation') }}"+"/IEX/"+$year+"/"+$month+"/"+$day;
                }
            });
        });
    </script>
